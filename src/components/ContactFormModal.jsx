...
export default function ContactFormModal({ isOpen, onClose }) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="bg-white w-full max-w-2xl border-4 border-black rounded-3xl p-6 md:p-8 shadow-neo relative flex flex-col my-8"
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 bg-custom-red text-white p-1.5 rounded-full border-2 border-black hover:scale-105 active:translate-y-0.5 cursor-pointer z-10"
              ...
              <div className="grid grid-cols-2 gap-3">
                {contactLinks.map((link) => (
                  <a
                    ...
                    onMouseEnter={() => audioSynth.playHover()}
                    className={`${link.color} py-2.5 rounded-xl border-2 border-black font-bold shadow-[2px_2px_0_rgba(0,0,0,1)] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-all flex items-center justify-center gap-2 cursor-pointer text-xs`}
                  >
                    {link.icon}
                    {link.label}
                  </a>
